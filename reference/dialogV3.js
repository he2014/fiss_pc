(function($) {
	...
	$.extend({
		...
		iframe: function(a, b, c, d, e, f, g, h, i) {
			...
			var n = $('<img style="width:23px;height:23px" src="/resource/static/image/myinfo/loading.gif"/>');
		...
		tip_alert:function(title,desc,is_show_close,auto_close_time){
			...
			var tip_id = 'tip_'+(new Date().getTime()),top_temp = window.tip_alert_div.height;
			...
			tip_alert_div.animate({opacity: '1',top: ($(window).scrollTop() + $(window).height() / 2 - tip_alert_div.height())+ "px"}, 1000, 'swing');
			if(auto_close_time > 0){
				setTimeout(function(){ html_obj.slideUp(function(){html_obj.remove();});}, auto_close_time * 1000);
			}
			
			$("#"+tip_id+" .close").click(function() {
				html_obj.slideUp(function(){html_obj.remove();});
				//html_obj.remove();
			});
		},
