...
(function($) {
    ...
    $.i18n.properties = function(settings) {
        ...
        var settings = this.settings = $.extend(defaults, settings);
        ...
        for (var i = 0; i < files.length; i++) {
            if (settings.language.length == 0) {
                loadAndParseFile(settings.path + files[i] + '.properties?' + FE_VERSION, settings);
            }
            if (settings.language.length >= 2) {
                loadAndParseFile(settings.path + files[i] + '_' + settings.language.substring(0, 2) + '.properties?'+ FE_VERSION, settings);
            }
            if (settings.language.length >= 5) {
                loadAndParseFile(settings.path + files[i] + '_' + settings.language.substring(0, 5) + '.properties?' + FE_VERSION, settings);
            }
        }
	...
	$.fn.safeHide = function() {
		var t = $(this);
		if (!t.hasClass(hiddenClass))
			t.addClass(hiddenClass);
		return t;
	};
}
)(jQuery);