...
(function(root, factory) {
	...
	function photoClip(container, option) {
		...
		var $img,
			...
			imgLoaded,img_type; //图片是否已经加载完成
		var Orientation = null;
		$file.attr("accept", "image/*");
		$file.change(function() {
			//var Orientation = null;
			img_type=this.files[0];
			if (!this.files.length) return;
			if (!/image\/\w+/.test(this.files[0].type)) {
				alert("图片格式不正确，请选择正确格式的图片文件！");
				return false;
			} else {
				EXIF.getData(img_type, function() {
				...
				fileReader.onload = function(e) {
					...
					//上传图片的尺寸验证
					if(kbs >2048){
						//console.log("max size 2048kb");
						$('.tips_detail').html('الصورة المرفقة غير صحيحة');
		...
		function clipImg() {
			...
			clipFinish.call($img[0], img_type);
		}
		...
		function compressImg(sourceImgObj, quality, angleOffset, outputFormat){
			...
			if (navigator.userAgent.match(/iphone/i)) {
				if(Orientation != "" && Orientation != 1){
					switch(Orientation){
						case 6:
							rotateImg(sourceImgObj,'left',cvs);
							break;
						case 8:
							rotateImg(sourceImgObj,'right',cvs);
							break;
						case 3:
							rotateImg(sourceImgObj,'right',cvs);//转两次
							rotateImg(sourceImgObj,'right',cvs);
							break;
					}
				}
